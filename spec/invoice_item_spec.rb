...
class InvoiceItemTest < Minitest::Test
  ...
  def test_invoice_item_can_query_id
    invoice_item = test_helper_invoice_item
    assert_equal 3, invoice_item.id
  end

  def test_invoice_item_can_query_item_id
    invoice_item = test_helper_invoice_item
    assert_equal 26351, invoice_item.item_id
  end

  def test_invoice_item_can_query_invoice_id
    invoice_item = test_helper_invoice_item
    assert_equal 12334771, invoice_item.invoice_id
  end
  ...
  def test_invoice_item_can_query_unit_price
    invoice_item = test_helper_invoice_item
    assert_equal 348.73, invoice_item.unit_price.to_f
  end
